function exp_data=trace_on_of_flashes_one_recording(logfile_fullname, flash_dur_s, tbefore_s, tafter_s)
...
% function  save_raw_ds_grating(rootDir, prfullname,logfullname, filedatestr) 
    ...
    tbefore_flash_s=0.5;
    tbefore_flash=round(tbefore_flash_s*sampling_rate);
    resps_all_on  = [];
    resps_all_off  = [];
    resps_all_beforeflash  = [];
    for i=1:nrep
        resps_all_on = cat(1,resps_all_on, Data(startend(i,1):startend(i,1)+ondur-1,1)');        
        resps_all_beforeflash = cat(1,resps_all_beforeflash, Data(startend(i,1)-tbefore_flash:startend(i,1)-1,1)');        
        if i<nrep || nrep==1
            resps_all_off = cat(1,resps_all_off, Data(startend(i,2)+1:startend(i,2)+1+offdur-1,1)');
        end
    end
    ...
    exp_data.trace.beforeflash_traces = resps_all_beforeflash;        
end
