...
function exp_data = compute_scanning_RF_OF_perfile(prfullname,logfullname)
    ...
    %% open ephys data    
    [Data, Text_header, filenameout, sampling_rate]=openpr_flatten_spikes(prfullname,0);
    ...
    %% find spikes and replace graded potential data with spike data in Data
    [spike_data, mean_spike] = get_spike_data(Data(:,1)');
    Data(:, 1) = spike_data';
...
%     V_frame_resp= cell2mat(arrayfun(@(i) mean(Data(bin_edges(i,1):bin_edges(i,2),1)),1:num_frame_registered, 'UniformOutput', false)')';
    V_frame_resp=cell2mat(arrayfun(@(i) sum(Data(bin_edges(i,1):bin_edges(i,2),1)),1:num_frame_registered, 'UniformOutput', false)')';
