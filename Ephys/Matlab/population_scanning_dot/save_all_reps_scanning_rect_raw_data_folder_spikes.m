...
% curfolder='\\fs.ist.ac.at\dfsgroup\joeschgrp\Vika\EPhys\shakB_project\Repository\FlpND\VS\VS1_4\210128';
    ...
    for i=1:nfiles
        ...
        rawdata_i=scanning_rect_one_recording_seprep_spikes(fulllog_file);
        ...
        for ri=1:rawdata_i.stim_info.nrep
            ...
            %save the rep
            if isempty(cellid)
                filename_i=['scaning_rect_spikes_',strain,'_',celltype,'_',datestr,'_rep_',num2str(repi)];
            else
                filename_i=['scaning_rect_spikes_',strain,'_',celltype,'_',datestr,'_',cellid,'_rep_',num2str(repi)];
            end
            fullfile_i=fullfile(resfolder,[filename_i,'.mat']);
            save(fullfile_i,'rep_data','-v7.3');
            make_figure_RF_one_rep(rep_data, resfolder, 'spike');
            repi=repi+1;
        end
    end
end