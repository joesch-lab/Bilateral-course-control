...
function raw_data = scanning_dot_one_recording_slow(logfile_fullname) 
    raw_data={};
...
function prfilename = find_pr_file_closest_date(logfilename)
    ...
    min_et=3600;
    prfilename=-1;
    %find the pr file closes by time
...
%% using lin interpolation find the timing of every frame
   ...
   frametiming=uint32(round(interp1(rfst,rft_st,allframesst,'linear','extrap')));
    ...
    %raw data
    exp_data.V_per_frame=V_frame_resp;
    exp_data.resp_bins = bin_edges;
    %stim data
    exp_data.stim_array=stim_arr;
    
    %RF data    
    exp_data.RFx = RFx;
    exp_data.RFy = RFy;
    exp_data.RFx_pos = RFx_pos;
    exp_data.RFx_neg = RFx_neg;
    exp_data.RFy_pos = RFy_pos; 
    exp_data.RFy_neg = RFy_neg; 
end
