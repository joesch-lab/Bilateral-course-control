...
%     figure, scatter(X_proj,Y_proj,'.');    
    ...
    %mean length of an arrow accross the columns and rows
    bin_hor=mean(bin_resp_deformed,'omitnan');
    bin_ver=mean(bin_resp_deformed,2,'omitnan');
    %length of a mean arrowacross columns and rows
%     bin_hor_x=mean(bin_dx_av(:,:,1),1);
%     bin_hor_y=mean(bin_dx_av(:,:,2),1);
%     bin_hor=(bin_hor_x.^2+bin_hor_y.^2).^0.5;
%     bin_ver_x=mean(bin_dx_av(:,:,1),2);
%     bin_ver_y=mean(bin_dx_av(:,:,2),2);
%     bin_ver=(bin_ver_x.^2+bin_ver_y.^2).^0.5;
