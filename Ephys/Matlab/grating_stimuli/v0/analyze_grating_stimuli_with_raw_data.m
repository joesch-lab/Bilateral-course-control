...
function  compute_grating(rootDir, prfullname,logfullname, filedatestr) 
     ...
     [filepath_1,filedate,~] = fileparts(filepath);
     ...
     date_year=str2double(filedate(1:2));
     date_month=str2double(filedate(3:4));
   ...
   %channel in Data with red frames
    ...
    ndir=8;
    dir_vector = [0:45:359];
    baseline_duration=round(0.5*sampling_rate);
    ...
    resps_all  = zeros(nrep,ndir);
    baseline_all  = zeros(nrep,ndir);
    ...
    for i=1:all_rec_n
        ...
        resps_all(repi,diri)  = mean(resps_all_raw(repi,diri,:));
        baseline_raw(repi,diri,:)  = Data(startend(1,i)-baseline_duration+1:startend(1,i),1);
        baseline_all(repi,diri)  = mean(baseline_raw(repi,diri,:));
    end
    
    resps_minus_baseline = resps_all-baseline_all;
    pd_i=1;
    nd_i=5;
    on_off=[baseline_duration+1,baseline_duration+min_grating_dur,...
        2*baseline_duration+min_grating_dur+1,2*baseline_duration+2*min_grating_dur];
    baseline_pd=baseline_raw(:,pd_i,:)-baseline_all(:,pd_i);
    baseline_pd=squeeze(mean(baseline_pd,1));    
    baseline_nd=baseline_raw(:,nd_i,:)-baseline_all(:,nd_i);
    baseline_nd=squeeze(mean(baseline_nd,1));
    resp_pd=resps_all_raw(:,pd_i,:)-baseline_all(:,pd_i);
    resp_pd=squeeze(mean(resp_pd,1));    
    resp_nd=resps_all_raw(:,nd_i,:)-baseline_all(:,nd_i);
    resp_nd=squeeze(mean(resp_nd,1));
    
    pd_nd_hor_cells=[baseline_pd',resp_pd',baseline_nd',resp_nd'];
    
    av_all=mean(resps_all,1);
    mean_traces=resps_all_raw-baseline_all;
    mean_traces=squeeze(mean(mean_traces,1));
    
    
    fig_base=fullfile(resfolder, name);
    make_polar_plot_direction_selectivity(av_all', mean_traces, dir_vector, name, fig_base);
    make_pd_nd_plot(pd_nd_hor_cells,on_off,sampling_rate, name,fig_base);
    
    matfile=fullfile(resfolder, [name,'_res.mat']);
    %save variables
    save(matfile, 'resps_all', 'baseline_all','resps_all_raw', 'baseline_raw', ...
        'resps_minus_baseline', 'pd_nd_hor_cells', 'on_off', 'sampling_rate', '-nocompression','-v7.3');    
end
...
function make_pd_nd_plot(pd_nd_trace,on_off, sampling_rate, title_str, figbase)
    figure('Position',[100,100,800,800]);
    ymin=-20;
    ymax=20;
    %title
    title_str={'PD and ND responses for ';title_str};
    
    %polar plot
    x=[1:length(pd_nd_trace)]/sampling_rate;
    plot(x,pd_nd_trace,'k'); hold on;
    on_off_plot=on_off/sampling_rate;
    on_off_plot=[on_off_plot;on_off_plot];
    on_off_plot_y=zeros(size(on_off_plot));
    on_off_plot_y(1,:)=ymin;
    on_off_plot_y(2,:)=ymax;    
    plot(on_off_plot,on_off_plot_y,'--','color',[0.3,0.3,0.3]); 
    ylim([ymin,ymax]);
...
%     legend(num2str(direction_vector(:)));
    ...
    title(title_str, 'fontsize', 13, 'FontWeight','bold', 'Interpreter','none');
    

    figsavename=[figbase,'_pd_nd.png'];
    saveas(gcf,figsavename,'png');     
    figsavename = [figbase,'_pd_nd.fig'];
    savefig(figsavename);
end
...
function make_polar_plot_direction_selectivity(av_total, mean_traces, direction_vector, title_str, figbase)
    figure('Position',[0,0,700,2000]);
    
    %title
    subplot('Position',[0.2,0.95,0.6,0.05]);
    text(0,0,title_str, 'fontsize', 13, 'FontWeight','bold','Interpreter','none');
    axis off;
...
%     ystr=num2str([minval+baseline_activity,baseline_activity,maxval+baseline_activity],'%.3f\n');
    ...
    figsavename=[figbase,'_polar_DS.png'];
    saveas(gcf,figsavename,'png');
    figsavename = [figbase,'_polar_DS.fig'];
    savefig(figsavename);
end
