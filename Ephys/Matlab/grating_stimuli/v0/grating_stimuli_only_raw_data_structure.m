...
parent_folder='\\fs.ist.ac.at\dfsgroup\joeschgrp\Vika\EPhys\shakB_project\Repository\FlpD\HS\HSN';

% parent_folder='\\istsmb3.ist.local\joeschgrp\Vika\EPhys\shakB_project\Repository\CantonS\HS\HSS';
...
for li =1:length(filelist)
    if  contains(filelist(li).name,'grating_stimuli','IgnoreCase', true) && contains(filelist(li).name,'.log','IgnoreCase', true)  
        ...
        try
            save_raw_ds_grating(parent_folder, prfile_fullname,logfile_fullname, filedatestr);
            close all;
        catch
...
function  save_raw_ds_grating(rootDir, prfullname,logfullname, filedatestr) 
    %% get celltype and strain type from the path
     ...
     date_year=num2str(datestr(1:2));
     date_month=num2str(datestr(3:4));
   ...
   %channel in Data with red frames
    ...
    exp_data.celltype=cell_type;
    exp_data.celltype=cell_grp;    
