...
class GeoAxes(Axes):
    ...
    def set_longitude_grid(self, degrees):
        ...
        self.xaxis.set_major_locator(
            FixedLocator(
                np.linspace(-np.pi, np.pi, number, True)))
    ...
    def set_latitude_grid(self, degrees):
        ...
        self.yaxis.set_major_locator(
            FixedLocator(
                np.linspace(-np.pi / 2.0, np.pi / 2.0, number, True)))
...
class SinusoidAxes(GeoAxes):
    """
    A custom class for the sinusoid projection, an equal-area map
    projection.
    """
    ...
    # user to select the projection,
    # i.e. ``subplot(111, projection='custom_hammer')``.
    name = 'custom_sinusoid'

    class HammerTransform(Transform):
        """
        The base sinusoid transform.
        """
        input_dims = 2
        output_dims = 2
        is_separable = False
        ...
        def __init__(self, resolution):
            """
            Create a new sinusoid transform.  Resolution is the number of steps
            to interpolate between each input line segment to approximate its
            path in curved Hammer space.
            """
            Transform.__init__(self)
...
# Now register the projection with matplotlib so the user can select
# it.
register_projection(SinusoidAxes)
...
if __name__ == '__main__':
    ...
    plt.subplot(111, projection="custom_sinusoid")
