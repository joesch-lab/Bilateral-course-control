...
def remove_saccades(opto_response_angular_speed, opto_response, saccade_peaks, window=[5, 5],
                    remove="syn", stim_period=600, filler_type='zero'):
    """
    ...
    opto_turns = np.apply_along_axis(lambda x: np.diff(x), axis=1, arr=opto_response)
    opto_turns = np.insert(opto_turns, 0, opto_response[:, 0], 1)
    new_opto_response_angular_speed = new_opto_response_angular_speed

    if remove == 'syn':
        indices = np.argwhere(saccade_peaks[:, stim_period:] > 0)
    elif remove == 'anti':
        indices = np.argwhere(saccade_peaks[:, stim_period:] < 0)
    elif remove == 'all':
        indices = np.argwhere(saccade_peaks[:, stim_period:] != 0)
